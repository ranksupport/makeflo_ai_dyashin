...
export const InHouse = memo(({ data }) => {
  ...
  return (
    ...
    >
      ...
      {data?.app_detail.provider.toLowerCase() === "textsplitter" &&
        data?.selectedEvent && (
          <SplitterConfig
            collapseConfiguration={data.app_status.collapseConfiguration}
            nodeOuter={nodeOuter}
            node_id={data.node_id}
            inHouseAppConfig={data.inHouseAppConfig}
      ...
      {(data?.app_status?.showTestButton === true ||
        data?.configResponses?.length > 0) && (
        <>
        <Flex
